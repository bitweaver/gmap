...
if (!empty($_REQUEST["save_copyright"])) {
    if( $result = $gContent->storeCopyright( $_REQUEST ) ) {
		...
		$mRet = "<copyright>"
		  ."<copyright_id>".$result->fields['copyright_id']."</copyright_id>"
		  ."<copyright_minzoom>".$result->fields['copyright_minzoom']."</copyright_minzoom>"
		  ."<bounds>".$result->fields['bounds']."</bounds>"
		  ."<notice>".$result->fields['notice']."</notice>"
		."</copyright>";
    }else{
