...
//wj: assume at some point we'll need this.
 ...
 */
	...
	function get_map($map_id) {
		...
		//SELECT Row from Table tiki_gmaps WHERE map_id == $map_id
		
			$query = "SELECT tm.*, 
				  	  FROM `".BIT_DB_PREFIX."tiki_maps` tm 
						INNER JOIN `".BIT_DB_PREFIX."users_users` uu ON (uu.`user_id` = tm.`user_id`)
			...
			$query = "SELECT tg.*, tc.*,
					  ...
					  FROM `".BIT_DB_PREFIX."tiki_gmaps` tg
						INNER JOIN `".BIT_DB_PREFIX."tiki_content` tc ON (tc.`content_id` = tg.`content_id`) $joinSql
					  ...
					  WHERE tg.`$lookupColumn`=? $whereSql";
...
//wj: Everything below here from Wiki version
class BitMap extends LibertyAttachable {
	var $mPageId;
	var $mPageName;


/*wj: looks like this is for getting data from tiki_content which we are not using.
	function BitMap( $pPageId=NULL, $pContentId=NULL ) {
	...
	**/
	function store( &$pParamHash ) {
		...
		if( $this->verify( $pParamHash ) && LibertyAttachable::store( $pParamHash ) ) {
			...
			/* wj: start comment out function not necessary for basic wiki loading */
			/*
			if( isset( $mailEvents ) ) {
...
#							break;
			...
			*/
			/* wj: end comment out function not necessary for basic wiki loading */
...
*/		}
    ...
    */
	function getListLink( $pPageHash ) {
		return BitMap::getDisplayUrl($pPageHash['title']);
	}
		...
		if( $gBitSystem->isPackageActive( 'wiki' ) ) {
			if( is_array( $pExistsHash ) ) {
				...
				if( count( $pExistsHash ) > 1 ) {
					...
					$ret = "<a title=\"$desc\" href=\"" .  BitMap::getDisplayUrl( $exists['title'] ) . "\">$pPageName</a>";
 ...
 * @package wiki
 */
class BitMapLib extends BitMap {
    function BitMapLib() {
		BitMap::BitMap();
    }
...
/**
 * the bitmaplib class
 * @global BitMapLib $bitmaplib
 */
global $bitmaplib;
// Perhaps someone overrode the bitmaplib class to do there own magic, and have alread instantiated...
if( empty( $bitmaplib ) ) {
