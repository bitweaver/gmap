...
<div class="admin gmap display">
	...
	<div class="body">
		...
		{if $smarty.request.preview}
			{include file="bitpackage:gmap/map_list.tpl"}
		{/if}
		...
		{form}
			...
			<table class="data">
				<caption>{tr}Gmap Marker History{/tr}</caption>
				<tr>
				...
				<tr class="odd">
					<td colspan="4">
						...
						&nbsp;&bull;&nbsp;{smartlink ititle="Source" marker_id=`$gContent->mGmarkerId` source="current"}
					</td>
				</tr>
				...
				{foreach from=$data item=item}
					...
					<tr class="{cycle values='even,odd'}">
						<td colspan="3">
							{smartlink ititle="View" marker_id=`$gContent->mGmarkerId` preview=`$item.version`}
							&nbsp;&bull;&nbsp;{smartlink ititle="Compare" marker_id=`$gContent->mGmarkerId` compare=`$item.version`}
							&nbsp;&bull;&nbsp;{smartlink ititle="Difference" marker_id=`$gContent->mGmarkerId` diff2=`$item.version`}
							&nbsp;&bull;&nbsp;{smartlink ititle="Source" marker_id=`$gContent->mGmarkerId` source=`$item.version`}
							{if $gBitUser->hasPermission( 'p_marker_rollback' )}
								&nbsp;&bull;&nbsp;{smartlink iurl="rollback.php" ititle="Rollback" marker_id=`$gContent->mGmarkerId` version=`$item.version`}
							{/if}
						</td>
						<td style="text-align:right;">
							{if $gBitUser->hasPermission( 'p_marker_remove' )}
